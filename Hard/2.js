function applyGravity(array) {
    ...
    const gravityApplied = Array.from({ length: cols }, (_, colIndex) => {
      ...
      const shiftedColumn = Array(emptySpaces).fill('-').concat(column.filter(cell => cell === '#'));
    ...
    const finalArray = Array.from({ length: rows }, (_, rowIndex) =>
      Array.from({ length: cols }, (_, colIndex) => gravityApplied[colIndex][rowIndex])
    );
  ...
  const result = applyGravity(array);
